...
RPCServer.prototype._send = function () {
  ...
  try {
    ...
    if (msg instanceof ReturnMessage) {
      ...
      this.logger.warn(
        "Encoding error " + e.message + " / recover error message.",
        e
      );
      this._send();
      return;
    ...
    this._send();
    this.logger.error(
      "Encoding error " + e.message + " / Could not recover the messaging."
    );
    return;
  }
  ...
  var len = buf.length;
  var bufok = this.socket.write(
    new Buffer(padRight(len.toString(16), "0", 6) + strBody, "utf8")
  );
  this.logger.debug(
